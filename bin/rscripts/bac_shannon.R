...
# Check if 'name' is in the column names
...
img<-ggplot(alpha_div, aes(x=' ', y=alpha_div))+
  ...
  geom_text( aes(y=quantile(alpha_div,0.50),
                 label='Clinically healthy
  dogs fall in this range'), check_overlap = TRUE, size=3.5, fontface = "bold", vjust = 1.3) +
 ...
 coord_flip() + labs(x='', y='') + scale_y_continuous(breaks=number_ticks(15)) +
